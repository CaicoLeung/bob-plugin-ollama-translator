...
8. Detail depth is user-selectable via the `wordDetail` menu — `fast` (core senses only, no additions) / `medium` (default: all senses + etymology/usage) / `full` (learner's-dictionary depth) — each tier is a distinct JSON prompt, and the tier is part of the result cache key **for word lookups only**, so switching tiers never serves the other tier's cached dict entry; text-translation entries stay shared across tiers.
